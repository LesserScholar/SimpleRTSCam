...
namespace SimpleRTSCam
{
    ...
    internal class SpectatorControlPatch : HarmonyPatch
    {
        ...
        {
            yield return AccessTools.Method(typeof(MissionGauntletSpectatorControl), "OnMissionTick");
    ...
    internal class UnlockCameraFromBoundariesPatch : HarmonyPatch
    {
        static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            for (int i = opsToRemove - 1; i < codes.Count; i++)
            {
                ...
                if (codes[i].opcode == OpCodes.Callvirt && codes[i].operand == (object)AccessTools.Method(typeof(IMissionDeploymentPlan), nameof(IMissionDeploymentPlan.HasDeploymentBoundaries)))
                {
    ...
    [HarmonyPatch(typeof(MissionGauntletOrderOfBattleUIHandler), "OnDeploymentFinish")]
    internal class MissionOrderOfBattleGauntletUIHandlerPatch : HarmonyPatch
    {
        static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            var toRemove = new List<CodeInstruction> {
                new CodeInstruction(OpCodes.Ldarg_0, null),
                new CodeInstruction(OpCodes.Ldfld, (object)AccessTools.Field(typeof(MissionGauntletOrderOfBattleUIHandler), "_orderOfBattleCategory")),
